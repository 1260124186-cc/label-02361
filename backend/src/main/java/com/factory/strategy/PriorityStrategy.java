// -*- coding: utf-8 -*-
package com.factory.strategy;

import com.factory.model.ProductionType;
import com.factory.model.ProductionTypeRegistry;

import java.util.List;
import java.util.NoSuchElementException;
import java.util.Random;

public class PriorityStrategy implements ProductionScheduleStrategy {

    private final Random random;

    public PriorityStrategy() {
        this(new Random());
    }

    public PriorityStrategy(Random random) {
        if (random == null) {
            throw new IllegalArgumentException("Random must not be null");
        }
        this.random = random;
    }

    @Override
    public int nextPID(ProductionTypeRegistry registry) {
        List<ProductionType> enabled = registry.getEnabled();
        if (enabled.isEmpty()) {
            throw new NoSuchElementException("No enabled production types available");
        }

        int totalPriority = enabled.stream()
                .mapToInt(ProductionType::getPriority)
                .sum();

        if (totalPriority <= 0) {
            List<Integer> ids = enabled.stream()
                    .map(ProductionType::getId)
                    .toList();
            return ids.get(random.nextInt(ids.size()));
        }

        int target = random.nextInt(totalPriority);
        int cumulative = 0;
        for (ProductionType pt : enabled) {
            cumulative += pt.getPriority();
            if (target < cumulative) {
                return pt.getId();
            }
        }

        return enabled.get(enabled.size() - 1).getId();
    }

    @Override
    public String getName() {
        return "priority";
    }
}
